...
import gui.create.CreateActFrame;
...
public class ActivityPanel extends JPanel implements ActionListener, FocusListener {
		...
		if (currentDay.getActivity(actId) != null) {
			...
			includeLunchLabel =
				new JLabel("Inkludera lunch (" +
									 currentDay.getActivity(actId).getLunchLength() + "min)");
